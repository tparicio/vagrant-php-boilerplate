...
if [ ! -d "/etc/mysql" ]; then
	...
	sudo apt-key adv --recv-keys --keyserver hkp://keyserver.ubuntu.com:80 0xcbcb082a1bb943db > /vagrant/logs/mariadb.log 2>&1
	sudo add-apt-repository -y "deb http://nyc2.mirrors.digitalocean.com/mariadb/repo/$4/ubuntu $5 main" > /vagrant/logs/mariadb.log 2>&1
	sudo apt-get update -y > /vagrant/logs/mariadb.log 2>&1
	...
	sudo apt-get install -y mariadb-server > /vagrant/logs/mariadb.log 2>&1
	...
	sudo cp /vagrant/config/mariadb-my.cnf /etc/mysql/my.cnf

	sudo service mysql restart > /vagrant/logs/mariadb.log 2>&1
fi
